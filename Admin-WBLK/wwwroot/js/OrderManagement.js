// Khai báo biến toàn cục cho phần tử thông tin thanh toán online
...
function addProductRow(product, quantity = 1) {
    ...
    if (!product) {
        ...
        const searchProduct = async function(value) {
            ...
            try {
                console.log(`Tìm kiếm sản phẩm với mã: ${value}`);
                const response = await fetch(`/OrderManagement/GetProductInfo?id=${encodeURIComponent(value)}`);
                const data = await response.json();
                console.log('Kết quả tìm kiếm sản phẩm:', data);
                
                if (data && data.success && data.tensanpham) {
                    ...
                    updateRowTotal(row);
                    updateOrderTotal(); // Cập nhật tổng tiền đơn hàng
                } else {
...
function updateOrderTotal() {
    ...
    console.log(`Đang cập nhật tổng tiền đơn hàng với ${rows.length} sản phẩm`);
    ...
    rows.forEach((row, index) => {
        ...
        if (quantityInput && priceElement && priceElement.dataset.price) {
            ...
            subtotal += rowTotal;
            console.log(`Sản phẩm ${index+1}: Số lượng=${quantity}, Đơn giá=${price}, Thành tiền=${rowTotal}`);
        } else {
            console.log(`Sản phẩm ${index+1}: Không tìm thấy thông tin số lượng hoặc đơn giá`);
        }
    });
    ...
    console.log(`Giảm giá: ${discount}, Tổng tiền: ${total}`);
    ...
    // Gọi hàm cập nhật tổng tiền của trang Create nếu có
    if (typeof updateTotalAmount === 'function') {
        console.log('Gọi hàm updateTotalAmount của trang Create');
        updateTotalAmount();
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded - Initializing payment method handling');
    
    // Lấy các phần tử cần thiết
    const paymentMethodSelect = document.getElementById('paymentMethod');
    onlinePaymentInfo = document.getElementById('onlinePaymentInfo');
    
    // Kiểm tra nếu các phần tử tồn tại
    if (paymentMethodSelect && onlinePaymentInfo) {
        console.log('Payment elements found - Initial payment method:', paymentMethodSelect.value);
        
        // Hiển thị hoặc ẩn thông tin thanh toán online dựa trên phương thức thanh toán ban đầu
        if (paymentMethodSelect.value === 'Paypal' || paymentMethodSelect.value === 'Momo') {
            onlinePaymentInfo.classList.remove('hidden');
        } else {
            onlinePaymentInfo.classList.add('hidden');
        }
        
        // Thêm sự kiện xử lý thay đổi phương thức thanh toán
        paymentMethodSelect.addEventListener('change', function() {
            console.log('Payment method changed to:', this.value);
            if (this.value === 'Paypal' || this.value === 'Momo') {
                onlinePaymentInfo.classList.remove('hidden');
            } else {
                onlinePaymentInfo.classList.add('hidden');
            }
        ...
        console.log('Payment elements not found:', 
                    'paymentMethod =', paymentMethodSelect ? 'found' : 'not found', 
                    'onlinePaymentInfo =', onlinePaymentInfo ? 'found' : 'not found');
    }
    
    // Xử lý hiển thị form thanh toán khi ở trang Edit
    const editPaymentMethod = document.getElementById('Phuongthucthanhtoan')?.value;
    if (editPaymentMethod && onlinePaymentInfo) {
        console.log('Edit page - Payment method:', editPaymentMethod);
        if (editPaymentMethod === 'Paypal' || editPaymentMethod === 'Momo') {
            onlinePaymentInfo.classList.remove('hidden');
        } else {
            onlinePaymentInfo.classList.add('hidden');
        }
    ...
    // Thêm kiểm tra mã thanh toán vào sự kiện submit form
    const form = document.getElementById('createOrderForm');
    if (form) {
        console.log('Form found, adding submit handler');
        form.addEventListener('submit', function(e) {
            console.log('Form submitted, validating payment code');
            if (!validatePaymentCode()) {
                console.log('Payment code validation failed');
                e.preventDefault();
                return false;
            }
            console.log('Payment code validation passed');
            return true;
        });
    } else {
...
// Thêm hàm kiểm tra mã thanh toán khi submit form
function validatePaymentCode() {
    ...
    console.log('Validating payment code:', 
                'method =', paymentMethod, 
                'input =', mathanhtoanInput ? (mathanhtoanInput.value || 'empty') : 'not found',
                'info visible =', onlinePaymentInfo ? !onlinePaymentInfo.classList.contains('hidden') : 'element not found');
    
    // Kiểm tra nếu phương thức thanh toán là Momo hoặc Paypal
    if ((paymentMethod === 'Momo' || paymentMethod === 'Paypal') && 
        onlinePaymentInfo && !onlinePaymentInfo.classList.contains('hidden')) {
